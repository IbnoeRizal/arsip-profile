...
import { MISI_UPDATE_BY_ADMIN, MISIONS_CREATE_BY_ADMIN } from "@/lib/authschema";
...
export async function PATCH(request,context) {
    try{
        ...
        const validated = MISI_UPDATE_BY_ADMIN.safeParse(rawdata);
        if(!validated.success)
            return NextResponse.json({data:validated.error},{status:st4xx.badRequest});

        const misions = await prisma.misi.update({
            where:{id},
            data:validated.data,
            select:{
                id:true,
        ...
        return NextResponse.json({data:misions},{status:st2xx.created});       
    }catch(e){
